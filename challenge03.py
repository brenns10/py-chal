...
Start: http://www.pythonchallenge.com/pc/def/equality.html
Hint: One small letter, surrounded by EXACTLY three big bodyguards on each of
its sides.

The page title suggests that we use the re module.  The image and hint suggest a
lower case character surrounded by exactly 3 upper case ones.  There are
multiple matches, and when you put the small characters from each one together,
you get the next url.  However, the next url is a PHP file.
...
def main():
    pattern = '[a-z][A-Z]{3}(?P<char>[a-z])[A-Z]{3}[a-z]'
    word = ''.join(m.group('char') for m in re.finditer(pattern, DATA))
    print(word)

    webbrowser.open('http://www.pythonchallenge.com/pc/def/%s.php' % word)
